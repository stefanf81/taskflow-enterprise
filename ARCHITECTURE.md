...
12. **Frontend-Backend Connection Reuse, ETag & Cache-Control (P1-3, BENCHMARKS.md §44)**: Configured Tomcat Keep-Alives (`server.tomcat.max-keep-alive-requests=100`) and Nginx HTTP/1.1 upstream keepalive pooling so proxied API requests reuse warm backend TCP connections. We also added a `ShallowEtagHeaderFilter` (restricted to GET requests only — POST/PUT/DELETE skip ETag buffering entirely) so the backend returns an instant `304 Not Modified` if the JSON payload hasn't changed. **Tiered HTTP `Cache-Control`**: `CacheControl.maxAge(5, TimeUnit.MINUTES).cachePublic()` for `GET /api/v1/catalog` / `GET /api/v1/barbers` / `GET /api/v1/reviews/public/barber-ratings` (5m public, aligns with `@Cacheable` 10m TTL, ETag 304 after expiry), `maxAge(30, TimeUnit.SECONDS).cachePrivate().mustRevalidate()` for `GET /api/v1/appointments/public/busy-slots` (30s private, volatile), and `noCache().cachePrivate().mustRevalidate()` for `GET /api/v1/appointments` (admin paginated) & `GET /api/v1/barbers/admin` (freshness required, ETag still allows 304).
...
14. **k6 Load Profiles (P2, BENCHMARKS.md §48)**: `k6/load.js` is the isolated-stack `ramping-vus` stress profile, **0→50 (30s) →200 (60s) →0 (30s)** `gracefulRampDown 10s`, with `http_req_failed rate<0.01`, `http_req_duration p(95)<500 p(99)<800`, and `checks rate==1.0`; its 200 VUs must not target the public deployment's 100 requests/minute per-IP limiter. CI instead runs `k6/probe.js`, a one-request/second cacheable-public-API probe with the same availability, cache-header, and latency gates. Browser Core Web Vitals gates live in `k6/browser.js`.
...
3.  **Client Application State via Zustand (`src/store/`):** JWT authentication credentials and user session metadata are handled by the lightweight `useAuthStore`; transient screen state stays local to screens/components or in TanStack Query where it represents server state.
