...
1.  **JVM & Garbage Collection (Multi-Arch Tuning)**: Migrated from GraalVM JIT to **Standard OpenJDK 21** utilizing **ParallelGC**.
    - **Local Apple Silicon (M4 Pro ARM64):** The heap is strictly fixed at 1GB (`-Xms1g -Xmx1g`). GC threads are locked to 10 (`-XX:ParallelGCThreads=10`) to prevent the macOS scheduler from spilling "Stop The World" pauses onto the slower Efficiency cores, and JVM vectorization (`-XX:+UseSIMDForMemoryOps`) is enabled to exploit chip memory bandwidth.
    - **Production Cloud (x64):** Hardware-pinned JVM arguments (like fixed core counts or AVX flags) have been eradicated in favor of **Cgroup-Aware Portability**. The heap dynamically scales to the orchestrator's limits (`-XX:MaxRAMPercentage=75.0`), and the JVM automatically queries the Kubernetes cgroup to allocate the mathematically correct number of `ParallelGCThreads` for its specific node, preventing both CPU starvation and oversubscription.
