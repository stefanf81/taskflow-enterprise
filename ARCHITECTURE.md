...
1.  **JVM & Garbage Collection (Multi-Arch Tuning)**: Migrated from GraalVM JIT to **Standard OpenJDK 21** utilizing **ParallelGC**. The heap is strictly fixed at 1GB (`-Xms1g -Xmx1g`) to prevent OS-level allocation pauses.
    - **Local Apple Silicon (M4 Pro ARM64):** GC threads are locked to 10 (`-XX:ParallelGCThreads=10`) to prevent the macOS scheduler from spilling "Stop The World" pauses onto the slower Efficiency cores, and JVM vectorization (`-XX:+UseSIMDForMemoryOps`) is enabled to exploit chip memory bandwidth.
    - **Production AMD Ryzen 5 (7430U x64):** GC threads are locked to 6 (`-XX:ParallelGCThreads=6`) to eliminate SMT contention on physical cores, and Advanced Vector Extensions 2 are activated (`-XX:UseAVX=2` using correct non-boolean integer JVM syntax) for high-performance memory vectorizations.
