# Architecture Decision Records (ADRs)

This directory records architectural decisions made during the development of TaskFlow. Each ADR describes a significant decision, the context in which it was made, and the consequences.

## Index

| ID | Title | Status |
|----|-------|--------|
| [ADR-001](ADR-001-virtual-threads-enabled.md) | Virtual Threads — Enabled Explicitly | Accepted |
| [ADR-002](ADR-002-parallelgc-selection.md) | ParallelGC vs G1GC | Superseded (by G1GC in production) |
| [ADR-003](ADR-003-denormalized-customer-name.md) | Denormalized Customer Name | Accepted |
| [ADR-004](ADR-004-redis-for-caching.md) | Redis for Distributed Caching | Accepted |
| [ADR-005](ADR-005-jwt-httponly-cookie.md) | JWT in HttpOnly Cookie | Accepted |
| [ADR-006](ADR-006-react-native-expo-mobile-migration.md) | Migration to React Native & Expo Mobile | Accepted |
| [ADR-007](ADR-007-mobile-bearer-authentication.md) | Dedicated Mobile Bearer Authentication | Accepted |
| [ADR-008](ADR-008-openapi-contract-baseline.md) | Reviewed OpenAPI Contract Baseline | Accepted |
| [ADR-009](ADR-009-admin-appointment-sse.md) | Admin Appointment Updates via Server-Sent Events | Accepted |
| [ADR-010](ADR-010-bounded-async-executor.md) | Bounded Async Executor | Accepted |
| [ADR-011](ADR-011-reference-data-caching.md) | Reference Data Caching (Barbers & Services) | Accepted |
| [ADR-012](ADR-012-lua-rate-limiter.md) | Lua-Atomic Rate Limiter | Accepted |
| [ADR-013](ADR-013-partial-slot-index.md) | Partial Unique Slot Index (Anti Double-Booking) | Accepted |
| [ADR-014](ADR-014-resolve-no-preference-at-booking.md) | Resolve "No Preference" Bookings to a Concrete Barber | Accepted |
| [ADR-015](ADR-015-bind-idempotency-replay.md) | Bind Idempotency-Key Replays to the Original Request | Accepted |
