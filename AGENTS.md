...
- **No-Preference Assignment (ADR-014)**: `No Preference (First Available)` is input-only; `BusySlotsService.findFirstAvailableBarber()` (uncached) resolves it to a scheduled, non-time-off, free barber inside the booking transaction, and the concrete `barberName`/`barber` FK are persisted. Never store the sentinel — the partial index cannot protect a fictitious barber name. `V26__assign_sentinel_appointments.java` repairs legacy rows (assign first eligible barber, else `DENIED`). No barber available → 400.
- **Idempotency Replay Binding (ADR-015)**: an `Idempotency-Key` replay is honored only for the same customer email + date/time/service/barber (sentinel = wildcard); verified replay → 200 via `AppointmentCreationResult.replayed()`, mismatch → `IdempotencyConflictException` → 409 with no booking body, fresh booking → 201. Header capped at 100 chars. Both the pre-check and the `23505` catch path verify ownership.
...
- **Lookbook Fix (P2 §49)**: `mobile/src/components/lookbook/LookbookGallery.tsx` replaces `FlatList scrollEnabled={false}` inside parent `ScrollView` (defeats virtualization) with plain `LOOKBOOK_DATA.map` + `View` + `Card`; parent `ScrollView` owns scroll, documented upgrade to `FlashList` when catalogue >50 items. CWV budgets in `k6/browser.js` tightened to `ttfb<800 fcp<1800 lcp<2500` (web.dev good) vs prior lenient `p95<2500 lcp<6000`; remote CI (`k6.yml`) runs with `ENFORCE_CWV=false` because a GitHub runner measures cross-region latency, so the CWV budgets only gate same-region/local runs while the functional wizard checks remain the remote gate.
- **PgBouncer Ceiling (P2 §50)**: `application-prod.properties` documents Hikari `maximum-pool-size=25` (`minimum-idle=10`) knee curve (`10→3015 RPS`, `25→4128 RPS +37% selected`, `50→4257 RPS +3%`). The deployment is intentionally single-replica; distributed SSE and PgBouncer transaction pooling are future prerequisites for scale-out.
- **glibc Tuning on Ubuntu Base (§51)**: backend base is `eclipse-temurin:21-jre-resolute` (Ubuntu 26.04 LTS, digest-pinned; Alpine→glibc re-baseline: throughput/latency parity, +59 MiB RSS, +40 MB image, 0 vs 6 fixable HIGH/CRITICAL on the local variants). Adopted `MALLOC_ARENA_MAX=2` in `docker-compose.yml` backend env (glibc-only knob: arenas default to 8×cores and throttle RSS; measured −72 MiB median with no throughput/GC cost; mirror in `homelab/TF`). Rejected: jemalloc preload (+165 MiB RSS, `libjemalloc2` deliberately not shipped) and `-XX:+UseTransparentHugePages` (local VM THP=`always` already collapses pages). Harness `scripts/benchmark-glibc-tuning.sh` (fails if 0 2xx — the public rate-limit bucket 429s a raw `hey` sweep); assertions in `GlibcTuningBenchmarkTest`.
...
- **PgBouncer ceiling**: The current deployment is one replica against PostgreSQL `max_connections=50`. Do not add replicas until distributed SSE and PgBouncer transaction pooling are implemented.
