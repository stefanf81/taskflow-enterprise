...
export class AppointmentStore {
  ...
  readonly stats = signal<AppointmentStats>({
    total: 0,
    pending: 0,
    approved: 0,
    denied: 0,
    ...
    this.todoService
        ...
        catchError((err) => {
          if (err.status === 401) {
            ...
            this.errorMessage.set(
              'Could not connect to the backend server. Please ensure the Spring Boot API is running.',
            );
          }
      ...
      .subscribe((data) => {
        ...
        if (
          ...
          this.errorMessage() &&
          this.errorMessage()!.startsWith('Could not connect')
        ) {
